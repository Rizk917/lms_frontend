...
export default function SecondselectStudent() {
  ...
  const getstudentinfo = async () => {
    axios
      .get(`http://127.0.0.1:8000/api/students/id/${state.student_id}`)
      .then((response) => {
      ...
      <div className="M2-container2">
        <div className="M2-center-container">
          ...
          <div className="M2-addadmins">
            <label className="M2-label-Addadmins" htmlFor="">
              First name
            </label>
            {studentinfo && (
              <input
                className="M2-input-Addamis"
                type="text"
                readOnly="readOnly"
                placeholder={studentinfo.First_Name}
              />
            )}
            <label className="M2-label-Addadmins" htmlFor="">
              Last name
            ...
            {studentinfo && (
              <input
                ...
                type="text"
                readOnly="readOnly"
                placeholder={studentinfo.Last_Name}
              />
            )}
            <label className="M2-label-Addadmins" htmlFor="">
              Phone Number
            ...
            {studentinfo && (
              <input
                ...
                readOnly="readOnly"
                type="text"
                placeholder={studentinfo.phone_number}
              />
            )}
          </div>
          ...
          <div className="M2-buttons">
            <Link
              className="M4-cancel-classes"
              to={
                state.class_id
                  ? `/classes/${state.class_id}/sections/${state.sectionid}`
                  : '/students'
              }
              state={state}
            >
